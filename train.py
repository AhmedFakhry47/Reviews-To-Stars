...
if __name__ == '__main__':
	data_x,data_y = load_data('Reviews.csv')
	train_x,train_y,test_x,test_y,vocab_size = preprocess(data_x,data_y,train_test_ratio=0.90)
	print(train_y[:20])
	#Hyper parameters
	...
	print('Shape 0:',train_x.shape[1])
	print('Shape 1:',vocab_size)
	print('Shape 2:',train_y.shape[1])
	...
	optimizer = tf.train.GradientDescentOptimizer(learning_rate=1e-3).minimize(loss=out.loss)
	...
	with tf.Session() as sess:
	    ...
	    for epoch in range(nepochs):
	        ...
	        avg_rec   	 = sess.run(out.accuracy,feed_dict={out.input_x:test_x[:1000],out.input_y:test_y[:1000],out.dropout_prob:0})
	        #train_acc    = sess.run(out.accuracy,feed_dict={out.input_x:train_x,out.input_y:train_y,out.dropout_prob:1})
